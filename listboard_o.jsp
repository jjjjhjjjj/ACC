...
if (key==null || keyword==null){
	strSQL = "SELECT count(*) FROM tbllogin";
}else{
	strSQL = "SELECT count(*) FROM tbllogiin WHERE " + key + " like '%" + keyword + "%'";
}
...
<center><font size='3'><b> 관리페이지 </b></font></TD>
...
<TABLE border='0' cellspacing=1 cellpadding=2 width='600'>      
	...
	<TR bgcolor='cccccc'>      
		...
		<TD><font size=2><center><b>주소</b></center></font></TD>      
...
if(lastRow > 0) {
	if(key==null || keyword==null){
		strSQL = "SELECT * FROM tbllogin WHERE num BETWEEN " + startRow + " and " + endRow + " ORDER BY num DESC";
		rs = stmt.executeQuery(strSQL);
	} else {
		strSQL = "SELECT * FROM tbllogin WHERE " + key + " like '%" + keyword + "%' ORDER BY num DESC";
		rs = stmt.executeQuery(strSQL);
	}
...
%>
	...
	<TR bgcolor='ededed'>     
		...
		<TD align=left>
			<a href="write11_output_o.jsp?num=<%=listnum %>">
			<font size=2 color="black"><%=name %></font></a>
		</TD>
		<TD align=center>    
			<a href="write11_output_o.jsp?num=<%=listnum %>">
			<font size=2 color="black"><%=zip %><%=address1 %><%=address2 %></font></a>     
		</TD>     
		<TD align=center><font size=2><%=phone %></font>
		</TD>     
		<TD align=center><font size=2><%=email %></font>     
	</TR>  
...
if(lastRow > 0) {
	...
	if(currentPage > 1) {
%>
		<a href="listboard11_o.jsp?pageNum=<%=currentPage-1%>">[이전]</a>	
<%	
	}
...
%>
		<a href="listboard11_o.jsp?pageNum=<%=i%>">[<%=i%>]</a>
<%
		}
	}
	if(lastPage > currentPage) {
%>
		<a href="listboard11_o.jsp?pageNum=<%=currentPage+1%>">[다음]</a>
<%
	}
}
%>  
...
<TABLE border=0 width=600>
	<TR>
		<TD align='center'>	
			...
			<FORM Name='Form' Method='POST' Action='listboard11_o.jsp' OnSubmit='return Check()'>
